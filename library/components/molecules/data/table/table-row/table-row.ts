...
export class TableRow<T> {
  ...
  cellEvent = output<ICellEvent<T>>();
  expandableRows = input(false);
  isExpanded = input(false);

  actionsEvent = signal<Map<string, ICellEvent<T>>>(new Map());

  expandedIcon = computed(() => (this.isExpanded() ? faChevronDown : faChevronRight));

  actionsHelper = new DataActionHelper<T>(this.data, this.route, this.actionsEvent);
  ...
  handleCellEvent(event: ICellEvent<T>) {
